...
# function EigenProblem(weakformₖ::MultiFieldFEFunction,weakformₘ::MultiFieldFEFunction,test::MultiFieldFESpace,trial::MultiFieldFESpace;
#   nev::Int64=10,which::Symbol=:LM,explicittransform::Symbol=:none,tol::Float64=10^(-6),
#   maxiter::Int64=100,sigma=0.0)
#   L(v₁,v₂) = 0.0*v₁+0.0*v₂
#   opK = AffineFEOperator(weakformₖ, L, test, trial)
...
#     tol=prob.tol,maxiter=prob.maxiter,sigma=prob.sigma)
#   # fₕs = Vector{CellField}(undef, prob.nev)
#   fₕs=Vector{MultiFieldCellField}(undef, prob.nev)
#   for m=1:prob.nev
#     # fₕ = FEFunction(prob.trial, Vec[:,m])
#     fₕ = MultiFieldFEFunction(prob.trial, Vec[:,m])
#     fₕs[m] = fₕ
#   end
#   ξ, fₕs
# end