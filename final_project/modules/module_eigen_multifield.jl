...
struct EigenProblem_multifield <: FEOperator
...
"""
function EigenProblem(weakformₖ::Function,weakformₘ::Function,trial::FESpace,test::FESpace;
  nev::Int64=10,which::Symbol=:LM,explicittransform::Symbol=:none,tol::Float64=10^(-6),
  maxiter::Int64=100,sigma=0.0)
  L(v) = 0.0
  opK = AffineFEOperator(weakformₖ,L,trial,test)
  opM = AffineFEOperator(weakformₘ,L,trial,test)
  K = opK.op.matrix
  M = opM.op.matrix
  op = EigenOperator(K, M)
  EigenProblem(test,trial,op,nev,which,explicittransform,tol,maxiter,sigma)
end

function EigenProblem_multifield(weakformₖ::Function,weakformₘ::Function,test::MultiFieldFESpace,trial::MultiFieldFESpace;
  nev::Int64=10,which::Symbol=:LM,explicittransform::Symbol=:none,tol::Float64=10^(-6),
  maxiter::Int64=100,sigma=0.0)
  L((v₁,v₂)) = 0.0
  # opK = FEOperator(weakformₖ,L,trial,test)
  # opM = FEOperator(weakformₘ,L,trial,test)
  opK = AffineFEOperator(weakformₖ,L,trial,test)
  opM = AffineFEOperator(weakformₘ,L,trial,test)
  K = opK.op.matrix
  M = opM.op.matrix
...
function solve_multifield(prob::EigenProblem)
  K = prob.op.stima
  M = prob.op.massma
  ξ,Vec = eigs(K,M;nev=prob.nev,which=prob.which,explicittransform=prob.explicittransform,
    tol=prob.tol,maxiter=prob.maxiter,sigma=prob.sigma)
  ...
  # fₕs_real = Vector{CellField}(undef, prob.nev)
  # fₕs_imag = Vector{CellField}(undef, prob.nev)

  # for m=1:prob.nev
  #   fₕ_real = FEFunction(prob.trial, real(Vec[:,m]))
  #   fₕs_real[m] = fₕ_real
  #   fₕ_imag = FEFunction(prob.trial, imag(Vec[:,m]))
  #   fₕs_imag[m] = fₕ_imag
  # end
  # ξ, fₕs_real,fₕs_imag
